...
## ADR-0035 — LANG-M18 只修 B-layer terminal/tool final reply 和 no-execute code fence polish

- Decision: 在 `LANG-M18-tool-terminal-chinese-and-codeblock-polish` 中只修改 `ops/lib/language_layer.py` 和 `ops/tests/test_language_layer.py`，新增 deterministic terminal/tool final-reply 中文句子映射，并在用户明确 `do not execute`/`不要执行` 且响应含 fenced code block 时删除代码块后的未标注执行结果推断；随后通过既有 `LANG-M6` exact allowlist 的 `hermes-ops run --phase LANG-M6 --risk service-change -- hermes gateway restart` 加载。
- Reason: M9 rerun 的剩余 B-layer blocker 只有两类：terminal/tool-backed final answer 主体仍大多为英文，以及 no-execute fenced-code 回复仍可能追加 `If it ran, it would print...` 这类推断结果。两者都可在 B-layer deterministic renderer 内做最小、可测试、非模型化修复。
...
## ADR-0043 — LANG-M25 接受 hookable-only B-layer 并提交上游 hook issue

- Decision: M25 最终决策为 `ACCEPTED_AND_SUBMITTED`。当前 B-layer 作为本地生产可用的 hookable final-output layer 被接受；真实 interim/pre-tool/status commentary gap 作为 upstream-hook 依赖记录。已向 `NousResearch/hermes-agent` 提交 `transform_interim_output` proposal issue：https://github.com/NousResearch/hermes-agent/issues/35264。
- Reason: M22-M24 证据显示 `transform_llm_output` 覆盖 final-output 路径且 B-layer 已稳定启用，但真实 interim commentary 经 `run_agent.py:_emit_interim_assistant_message` 到 `gateway/run.py:_interim_assistant_cb` 后直接进入 commentary/status delivery，不调用现有 transform hook。M25 用户授权要求接受当前本地 scope，并在目标、认证、网络明确可用时提交 upstream proposal；`gh auth status`、`gh repo view NousResearch/hermes-agent`、`git ls-remote` 和 issue read-back 均通过。
